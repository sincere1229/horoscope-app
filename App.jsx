...
// ── メインアプリ ───────────────────────────────────────
// ── 既存の天文計算・SIGNS・PLANET_INFO・HoroscopeChartはそのまま残す ──
...
const LANG = {
  ja: {
    ...
    appSub: "星があなたの物語を語る",
    lead: "生年月日を入力してください",
    labelName: "お名前（ニックネームOK）",
    placeholderName: "例：恵子",
    labelBirth: "生年月日",
    placeholderYear: "1966", placeholderMonth: "7", placeholderDay: "11",
    labelHour: "出生時刻（わからなければ12時のまま）",
    labelLocation: "出生地（主要都市を選ぶか緯度経度を入力）",
    latPlaceholder: "緯度 例: 35.6762",
    lngPlaceholder: "経度 例: 139.6503",
    hourHint: "時（0〜23）",
    yearMonthDay: "年 ／ 月 ／ 日",
    btn: "🌌 ホロスコープを鑑定する",
    loading: "星を読んでいます...",
    errRequired: "名前と生年月日を入力してください",
    ...
    buildPrompt: (name, sunSign, moonSign, ascSign, planetDesc) =>
      `あなたは本格的な西洋占星術師です。
以下のホロスコープデータをもとに、${name}さんへの詳細な鑑定を行ってください。
【基本データ】
...
SNS: （X投稿用キャプション。絵文字たっぷり・共感性高く・120文字以内）`,
  ...
  en: {
    appTitle: "AI Horoscope",
    appSub: "The stars tell your story",
    lead: "Enter your birth details",
    labelName: "Your name (nickname OK)",
    placeholderName: "e.g. Keiko",
    labelBirth: "Date of birth",
    placeholderYear: "1966", placeholderMonth: "7", placeholderDay: "11",
    labelHour: "Birth time (leave as 12 if unknown)",
    labelLocation: "Birthplace (select a city or enter coordinates)",
    latPlaceholder: "Latitude e.g. 35.6762",
    lngPlaceholder: "Longitude e.g. 139.6503",
    hourHint: "hour (0–23)",
    yearMonthDay: "Year / Month / Day",
    btn: "🌌 Read My Horoscope",
    loading: "Reading the stars...",
    errRequired: "Please enter your name and date of birth",
    ...
    todayHead: "☀️ Today's Forecast",
    luckyHead: "🌟 Lucky",
    messageHead: "💫 Message from the Universe",
    snsHead: "📱 Caption for SNS",
    shareHead: "📲 Share",
    copy: "📋 Copy",
    copied: "✓ Copied!",
    ...
    buildPrompt: (name, sunSign, moonSign, ascSign, planetDesc) =>
      `You are a professional Western astrologer.
Please provide a detailed horoscope reading for ${name} based on the following chart data.
[Chart Data]
    ...
    try {
      ...
      const planetDesc = Object.entries(chart.planets)
        ...
        .map(([k, v]) => {
          ...
          const label = PLANET_INFO[k]?.label || k;
          return lang === "ja"
            ? `${label}：${sign.name}${(v % 30).toFixed(1)}°`
            : `${label}: ${sign.en} ${(v % 30).toFixed(1)}°`;
        })
        .join(lang === "ja" ? "、" : ", ");
        ...
        {!result ? (
          <div style={s.card}>
            ...
            <div style={s.field}>
              ...
              <div style={{display:"flex", gap:6, flexWrap:"wrap", marginBottom:10}}>
                {L.cities.map(c=>(
                  <button key={c.name} className="city-btn" onClick={()=>setForm({...form, lat:String(c.lat), lng:String(c.lng)})}
          ...
          <div style={{opacity:show?1:0, transform:show?"translateY(0)":"translateY(20px)", transition:"all 0.6s ease"}}>
            <div style={s.resultHeader}>
              ...
              <div style={s.signRow}>
                {[
                  {sign:result.chart.sunSign,  label:L.sunLabel},
                  {sign:result.chart.moonSign, label:L.moonLabel},
                  {sign:result.chart.ascSign,  label:L.ascLabel},
                ].map(({sign,label})=>(
            ...
            <div style={s.tabBar}>
              {TABS.map(t=>(
                <button key={t.id} className={`tab-btn ${activeTab===t.id?"tab-on":""}`}
                  onClick={()=>setActiveTab(t.id)} style={s.tabBtn}>{t.label}</button>
              ))}
            </div>
